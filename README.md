...
| Effects · Fill · Border · radius · style · weight · Transparency · size · position | the glow's softness, the outer halo and the presenter-only shimmer, then seven effect groups — **Common Effects** (the very effects the *text background* wears — Shadow, Pop, Lift, Float, Long shadow, Glow, Halo, Neon, Inner shadow, Inner glow, Bevel, Emboss, Gloss, Sheen, Spotlight, Stripes, Dots, Grid, Checker, Glass, Blur, the fades, Ring, Offset outline, Sticker, Stack, the accent bars and Corner fold — the shared `lib/shapeEffects` engine paints the plate exactly as it paints a text plate, each by intensity and colour, switching effects keeping both), **Shadow Effects** (twelve shadows on X · Y · Blur · Spread · Opacity · Colour), **Glow & Light** (twelve lights), **Depth / 3D** (twelve depths), **Modern Effects** (twelve finishes), **Shape Effects** (the corners and the plate's distortions) and **Decorative Effects** (fourteen decorations) — one effect wears at a time in each group, every group falls back to off, **and every colour in the card comes from the shape**: an effect's Colour field starts on *Auto*, which is the plate's own paint (`bannerFxColor()` — the solid fill, or the gradient's first stop), so an auto shadow, light, finish or decoration is repainted the moment the fill is, and picking a colour freezes that one effect to it; each tile is a miniature of the deck's own plate wearing its effect — the same `bannerCss()` the Shape card's tiles and the text background's preview run, so a tile cannot disagree with the slide; the body's paint (solid or the shared gradient builder — the multilayer-gradient silhouettes stack a second paint over whatever it is); the outline on a layer of its own; the corners (a cut silhouette keeps its straight edges, a masked silhouette keeps its curves — the mask decides, tab and arch round the top only); the two transparencies; and the plate's free px box and nudge on the board — while the height is automatic, the round and freehand silhouettes (circle, oval, blobs, cloud, strokes…) stretch it by their own factor, so a circle plate reads round without a hand-sized box |

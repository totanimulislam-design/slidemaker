...
one **Border** card (colour · style · weight · transparency · corners · glow),
...
**Design presets** · **Shape** · Effects · Fill · **Border** · Transparency ·
Banner size · Banner position · the eye · **Default**. The whole outline is
ONE button: its card holds the line's colour, style, weight, transparency, the
corner radius and the glow (see **Border** below). Everything the plate is made
of lives in `src/lib/banner.ts`; the same `bannerCss()` paints the board, the
panel previews, the thumbnails and the export.
...
| Effects · Fill · Transparency · size · position | the glow's softness, the outer halo and the presenter-only shimmer, then **seven effect categories**, the way a design tool files them — **Shadow** (twelve shadows on X · Y · Blur · Spread · Opacity · Colour: Drop · Soft · Hard · Long · Inner · Floating · Offset · Coloured · Double · Surround · Layered · Cast), **Glow** (eight lights on Type · Blur · Intensity · Colour: Outer · Inner · Neon · Soft · Halo · Backlight · Aurora · Outline Glow), **Blur** (eight blurs on Type · Blur · Intensity · Direction · Colour: Soft · Gaussian · Backdrop · Motion · Zoom · Feather · Bloom · Frosted), **Glass** (seven panes on Type · Blur · Intensity · Tint: Glassmorphism · Frosted · Acrylic · Blur background · Transparent · Tinted · Glass edge), **Bevel** (seven edges on Type · Depth · Blur · Angle · Colour: Bevel · Inner · Outer · Emboss · Ridge · Groove · Pillow — 0° is light from straight above and the lit edge walks with the angle), **3D** (nine depths on Type · Depth · Blur · Angle · Colour: Extrusion · Depth · Layered · Perspective · Tilt · Pop out · Raised · Pressed · Isometric — the slabs run the way the angle points, the turns tip the plate in space), **Highlight** (ten lights on Type · Blur · Intensity · Angle · Colour: Highlight · Inner · Outer · Edge · Gloss · Sheen · Shine · Reflection · Spotlight · Rim light) — plus **Decorations** (twenty-two patterns, textures and accents on Intensity · Colour, the patterns and the accents painted by the same `lib/shapeEffects` engine a text plate wears), **Finishes** (the modern surfaces — grain, soft gradient, mesh, holographic, metallic, duotone, soft UI — on Intensity · Blur · Tint) and **Shape effects** (the corners — shared or four of their own — and the plate's own distortions: stretch, wave, curve, slant, skew, rotation, the two flips); every category's **Type** row names its family and the tiles *are* the picker (one effect of a family at a time, the decorations and the distortions stack), every category falls back to off, **and every colour in the card comes from the shape**: an effect's colour field starts on *Auto*, which is the plate's own paint (`bannerFxColor()` — the solid fill, or the gradient's first stop), so an auto shadow, light, pane, edge or decoration is repainted the moment the fill is, and picking a colour freezes that one effect to it; each tile is a miniature of the deck's own plate wearing its effect — the same `bannerCss()` the Shape card's tiles and the text background's preview run, so a tile cannot disagree with the slide; a deck written before the categories keeps its effect — `bannerEffectsOf()` folds the old Common Effects, the old Depth / 3D card (the bevelled edges to **Bevel**, the extrusions and the turns to **3D**), the old Modern glass family (to **Glass**), the old glow card's highlights (to **Highlight**) and the old decorative highlights into the category each belongs to, and never overwrites one the deck already uses; the body's paint (solid or the shared gradient builder — the multilayer-gradient silhouettes stack a second paint over whatever it is); the shape's own transparency (the outline's lives in the Border card); and the plate's free px box and nudge on the board — while the height is automatic, the round and freehand silhouettes (circle, oval, blobs, cloud, strokes…) stretch it by their own factor, so a circle plate reads round without a hand-sized box |
| **Border** — ONE button, one card for the whole outline | **Outline the plate** (on / off) · **Border colour** — a click opens the same Solid / Gradient colour card the text colour uses (the document colours, the Canva swatches, the wheel, the hex field) as its own pop-up stacked on the Border pop-up; its ✕ closes only that card and the Border pop-up stays, and a second colour stacks on top of the first the same way · **Border style** (*Solid · Dashed · Dotted · Double · None*, as pictures) · **Border weight** (no ceiling) · **Border transparency** (the line's own, apart from the shape's) · **Border radius** — *All corners* (one bar, `radius`) or *Each corner* (top-left · top-right · bottom-right · bottom-left on bars of their own, `corners` + `cornersIndependent`: flipping it on starts from the corners the plate paints right now, so nothing moves, and *All corners* hands back the one radius; a cut or masked silhouette and the full ovals have no corner to round, and Effects ▸ Shape Effects' own corners still win — the card says so, with a button to take them back) · **Border glow** — on / off · size · strength · colour (*Auto* = the line's own colour; a colour of its own opens the same stacked Solid / Gradient card): a `drop-shadow` filter on the outline's own layer (`border.glow`), so the light follows the dashes, the dots, the corners and the line's transparency (on a cut or masked silhouette it stays inside the cut). Picking a colour, a style, a weight or the glow brings back a line that was off, the line always rounds the very corners the body wears, and in the inspector the same card opens its colour card as a pop-up of its own |
...
- **Blur is its own channel.** The outer halo, the **Blur** category, the glows'
  softness and the decorations that need one each say which layer they blur —
  the body's own `filter`, a blurred copy behind it (`data-banner-layer`), or
  the board through a `backdrop-filter` on the pane — so a blur can always be
  told apart from the paint around it (and a test can too). The two feathered
  highlights (the highlight blob and the swoosh highlight) carry their blur
  *inside* the mask's own SVG, so they stay a mask, not a filter.
- **A cut never outgrows the plate.** A bevel that reaches down a side is
  ...
  plate's own corners.
- **An effect wears the plate's colour.** Nothing in the Effects card brings a
  black or a white of its own: `bannerFxColor()` is the shape's paint, and every
  group's Colour field is on *Auto* until the teacher picks over it, so the
  shadow, the light, the finish and the decoration are lighter / darker passes
  of the plate and follow it when it is repainted. The tiles say the same thing
  twice over — each one is `BannerPlatePreview`, the deck's plate at
  `shrink` × on a miniature board, dressed with that effect, so an effect's px
  offsets, blurs and slabs keep their proportion to the body instead of being
  measured out at tile scale.

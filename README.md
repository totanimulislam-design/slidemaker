...
| **Border** — ONE button, one card for the whole outline | **Outline the plate** (on / off) · **Border colour** — a click hands the pop-up over to the same colour card every other colour on the bar opens (the document colours, the Canva swatches, the wheel, the hex field — solid colours, since a line is one flat colour), with **← Border** to come back and the Border button still lit · **Border style** (*Solid · Dashed · Dotted · Double · None*, as pictures) · **Border weight** (no ceiling) · **Border transparency** (the line's own, apart from the shape's) · **Border radius** — *All corners* (one bar, `radius`) or *Each corner* (top-left · top-right · bottom-right · bottom-left on bars of their own, `corners` + `cornersIndependent`: flipping it on starts from the corners the plate paints right now, so nothing moves, and *All corners* hands back the one radius; a cut or masked silhouette and the full ovals have no corner to round, and Effects ▸ Shape Effects' own corners still win — the card says so, with a button to take them back) · **Border glow** — on / off · size · strength · colour (*Auto* = the line's own colour; a colour of its own opens the colour card too): a `drop-shadow` filter on the outline's own layer (`border.glow`), so the light follows the dashes, the dots, the corners and the line's transparency (on a cut or masked silhouette it stays inside the cut). Picking a colour, a style, a weight or the glow brings back a line that was off, the line always rounds the very corners the body wears, and in the inspector the same card opens its colour card in place |

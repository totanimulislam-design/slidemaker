...
import { visibleStack, type LayerRef } from "./lib/layers";
import {
  ELEMENT_FIELD,
  anyGrouped,
  buildScene,
...
import { convertMode, measureElement, measurePart } from "./lib/geometry";
...
export default function App() {
  ...
  /**
   * The CANVAS SELECTION — one single list of object keys ("element:title",
   * "part:optionText:2", "shape:s12"…). Built-in elements, built-in parts and
   * drawn shapes all live in this ONE list, so a group, a marquee or a
   * multi-selection can mix any of them and every gesture works on the same
   * list. There are no separate element/part/shape selection stores.
   */
  const [selectedKeys, setSelectedKeys] = useState<ObjKey[]>(["element:title"]);
  /** the primary object of the selection — what the inspector / toolbar edit */
  ...
  const curTheme = effectiveTheme(deck, curSlide);
  const curHeader = effectiveHeader(deck, curSlide);

  /**
   * The full OBJECT MODEL of the current slide (lib/scene.ts): every built-in
   * element, built-in part and drawn shape with its group tag, z-order and
   * edit route. Selection expansion, hit routing, the toolbar and the keyboard
  ...
  const scene = useMemo(
    () =>
      buildScene({
        ...
        slide: curSlide,
        header: curHeader,
        background: effectiveBackground(deck, curSlide),
  ...
  /** the selection as layer refs (parsed keys), in selection order */
  const selectedRefs = useMemo<LayerRef[]>(
    () => selectedKeys.map(parseObjKey).filter((r): r is LayerRef => !!r),
    [selectedKeys],
  );
  /** drawn shapes / built-in parts / elements of the selection (derived views) */
  const selectedShapes = useMemo(
    () => selectedRefs.filter((r) => r.kind === "shape").map((r) => r.id),
    [selectedRefs],
  );
  const selectedParts = useMemo(
    () => selectedRefs.filter((r) => r.kind === "part").map((r) => r.id),
    [selectedRefs],
  );
  const selectedEls = selectedRefs.filter((r) => r.kind === "element").map((r) => r.id as ElementId);
  ...
  /** writes one selection into the store and points the inspector at it */
  const applySelection = useCallback(
    (refs: LayerRef[], opts?: { exact?: boolean }) => {
      // a click on one group member selects the whole group (unless exact)
      const expanded = opts?.exact ? refs : expandSelection(scene, refs);
      setSelectedKeys(expanded.map(objKey));
      // Which member does the inspector point at? (see primaryOf in lib/scene.ts)
      const primary = primaryOf(scene, expanded);
      ...
      if (primary.kind === "element") {
        setForceTab("layout");
        setActiveField(ELEMENT_FIELD[primary.id]);
        return;
      }
  ...
  /**
   * Canvas selection callback (the interaction layer speaks object keys) —
   * groups arrive expanded unless it is an exact dig-in.
   */
  const selectKeys = useCallback(
    (keys: ObjKey[], opts?: { exact?: boolean }) =>
      applySelection(
        keys.map(parseObjKey).filter((r): r is LayerRef => !!r),
        opts,
      ),
    [applySelection],
  );
  ...
  const selectShapeIds = useCallback(
    (ids: string[]) => applySelection(ids.map((id) => ({ kind: "shape", id }) as LayerRef)),
    [applySelection],
  );
  ...
  /** layers-panel / Tab-cycling pick: always exact, so one member of a group stays reachable */
  const selectLayer = useCallback(
    (ref: LayerRef) => applySelection([ref], { exact: true }),
    [applySelection],
  );
  ...
  /**
   * Double-click on a NON-text object (logo, frame, background layer, bullet,
   * banner, shape…): open that object's edit options in the inspector — the
   * canvas equivalent of picking the right panel by hand.
   */
  const openObjectOptions = useCallback((ref: LayerRef, field: SlideField | null) => {
    if (ref.kind === "shape") {
      setForceTab("shapes");
      return;
    }
    if (ref.kind === "element") {
      if (ref.id === "logo") {
        // the logo is a bitmap: show its edit options in the Header panel
        setForceTab("header");
        setActiveField("logo");
        return;
      }
      if (ref.id === "bullet") {
        setForceTab("questionBullet");
        setActiveField("question");
        return;
      }
      setForceTab("layout");
      setActiveField(field ?? ELEMENT_FIELD[ref.id]);
      return;
    }
    const info = partInfo(ref.id);
    setForceTab(info.tab);
    if (field ?? info.field) setActiveField((field ?? info.field)!);
  }, []);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (mod && e.shiftKey && !e.altKey && e.key.toLowerCase() === "a") {
        e.preventDefault();
        applySelection(currentStack.map((l) => parseObjKey(l.id)).filter((r): r is LayerRef => !!r));
        return;
      }
      ...
      if (e.key === "Tab" && !mod && currentStack.length && slideShapeCount > 0) {
        e.preventDefault();
        const selKeys = new Set<string>(selectedKeys);
      ...
      if (selectedShapes.length) {
        if (e.key === "Delete" || e.key === "Backspace") {
          ...
          applySelection(selectedRefs.filter((r) => r.kind !== "shape"), { exact: true });
          return;
        }
  ...
  const selectedGrouped = useMemo(
    () => (selectedRefs.length ? anyGrouped(scene, selectedRefs) : false),
    [selectedRefs, scene],
  );
        ...
        <div className="flex min-h-0 flex-1">
              ...
              <>
                ...
                <Stage>
                  <Slide
                    ...
                    onSelect={selectKeys}
                    onGeo={moveLayers}
                    onGroup={(refs) => groupLayers(refs)}
                    onUngroup={(refs) => ungroupLayers(refs)}
                    onOpenOptions={openObjectOptions}
                    onTextChange={setFieldText}
                    onShapeText={(id, text) => slide && updateShapeOnSlide(id, { text }, slide.id)}
                    onFrameWidth={(width) =>
                      scopedTheme({ frame: { ...currentTheme.frame, width } }, "frame-width")
                    }
                  ...
                  {selectedLayer && (
                    <>
                      ...
                      {(["back", "backward", "forward", "front"] as ZOp[]).map((op) => {
                        const enabled = canMove(currentStack, objKey(selectedLayer), op);
                        return (
                          <button
                            key={op}
                            disabled={!enabled}
                      ...
                      {selectedParts.length > 0 && (
                        <Btn
                          ...
                          onClick={() => {
                            ...
                            applySelection(selectedRefs.filter((r) => r.kind !== "part"), { exact: true });
                          }}
                        >
                          ⟲ Reset position
                        </Btn>
          ...
          <Inspector
            ...
            shapes={{
              ...
              onSelect: (id) => applySelection(id ? [{ kind: "shape", id }] : [], { exact: true }),
              ...
              onRemove: (id) => {
                removeShape(id);
                applySelection(selectedRefs.filter((r) => !(r.kind === "shape" && r.id === id)), { exact: true });
              },
              onDuplicate: (id) => {

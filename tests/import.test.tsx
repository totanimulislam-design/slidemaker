/**
 * Imported pages become SEPARATE, PLAIN slides of the user's own.
 *
...
import { STUB_PAGES } from "./pdf-stub";
...
/** the event lib/pdf.ts broadcasts when a PDF / PPTX is handed to the app */
const OPEN_PDF_EVENT = "slidemaker:open-pdf";
...
/** hand a file to the app: the same event a drop on the board dispatches */
const upload = async (name: string, type: string) => {
  const file = new win.File(["not really a document"], name, { type });
  act(() => {
    win.dispatchEvent(new CustomEvent<File>(OPEN_PDF_EVENT, { detail: file }));
  });
  await tick(60);
};
